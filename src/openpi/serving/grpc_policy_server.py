...
使用方法:
    # 启动服务器 (等待 Client 配置)
    python -m openpi.serving.grpc_policy_server --port 50051
...
logger = logging.getLogger("openpi.grpc_server")
...
class OpenPiModelInference:
    ...
    def action_horizon(self) -> int:
        ...
        if self.train_config and hasattr(self.train_config, 'model'):
            return getattr(self.train_config.model, 'action_horizon', 10)
        return 10
    ...
    def action_dim(self) -> int:
        """获取 action 维度"""
        if self.train_config and hasattr(self.train_config, 'model'):
            return getattr(self.train_config.model, 'action_dim', 16)
        return 16
    ...
    def predict(self, observation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            observation: 观测字典，包含:
                ...
                - "observation.images.{camera}": 图像数据
                - "prompt": 语言指令
...
class OpenPiInferenceServicer(pb2_grpc.OpenPiInferenceServiceServicer):
    ...
    def _build_observation_dict(self, request: "pb2.Observation") -> Dict[str, Any]:
        ...
        OpenPi 期望的格式:
        - "observation.state": (state_dim,) numpy array
...
class GrpcPolicyServer:
    ...
    def start(self):
        ...
        if self.config_name:
            logger.info(f"预加载模型: {self.config_name}")
        else:
